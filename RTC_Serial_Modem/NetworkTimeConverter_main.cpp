...
char foundchar[28];     // small buffer that works as a shift register to hold only the command for comparison
char SerialInputArray[30];
...
int counter = 0;
bool ready2read = false;        // 0=false, 1=true
bool newDataFromSerial = false; // 0=false, 1=true
...
void storeSerialData2Arrays()
{
    ...
    rtc_tz[2] = '\0';
    Serial.println("->storeSerialData2Arrays-DONE-");
}
...
void getDataFromSerial()
{
    ...
    if (Serial1.available() > 0)
    {
        ...
        {
            Serial.println("+CCLK found in array.start with first element in array");
            pointingfinger = 0;
            ready2read = true;
        ...
        if (ready2read == true)
        {
            ...
            while (foundchar[0] != '\n') // last element after the AT-Command feedback is new line
            {
                ...
                if (Serial1.available() > 0)
                {
                    ...
                    serialdata[pointingfinger + 1] = 0; // clean position
                    pointingfinger++;                   // set pointing finger to next step
                    foundchar[0] = foundchar[1];
                    foundchar[1] = foundchar[2];
                    foundchar[2] = foundchar[3];
                    foundchar[3] = foundchar[4];
            ...
            ready2read = false;
            Serial.println("");
            Serial.println("successfully r/w data.");
            Serial.println("");
        }
    }
    else
...
void readFromArray2TimeVariables()
{
    ...
    Serial.println("");
    Serial.println("->readFromArray2TimeVariables-DONE-");
    Serial.println("");
}
...
void convertNetworkTime2Integer()
{
    if (newDataFromSerial == false)
    {
        Serial.println("try to get data from serial.");
        getDataFromSerial();
        delay(50);
    }
